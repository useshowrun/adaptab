...
export async function registerBootstrapTools(): Promise<"registered" | "already_registered" | "unsupported"> {
  ...
  try {
    const registrations = [modelContext.registerTool({
      ...
      description:
        "Resolve a target page URL and task intent to the smallest compatible AdapTab WebMCP adapter. This checks the public catalog plus the signed-in owner's private library, without accessing the target page. Follow the returned executionPolicy, textual guidance, and evidenced limits; when additionalTabsRequired is false, reuse the resolved tab and do not open resource URLs as pages.",
      inputSchema: {
        type: "object",
        properties: {
