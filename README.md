...
[`/start`](https://adaptab.netlify.app/start) catalog. Both register the same
four WebMCP tools, so `/start` works as a convenient fallback. `/bootstrap`
remains the preferred agent route because it loads no React UI, catalog list,
or workspace status before registration. A capable agent may keep it in a
background or hidden browsing context, but it is not page-free: closing or
replacing that document removes its registered tools.
